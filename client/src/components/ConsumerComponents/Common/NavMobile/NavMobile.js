...
const NavMobile = () => {
    ...
    const handleShowFunctionAccount = () => {
        if(!auth.username) {
            ...
            const elementNavOption = document.querySelector('.nav-mobile__option-account');
            elementNavOption.style.display = 'block';
        }
    }
    ...
    return (
        <div className="nav-mobile">
            {arrayOptions.map((option, index) => (
                <li key={index} className="nav-mobile__btn" onClick={() => {
                    handleLoadingPage(0.5)
                    setTimeout(() => { window.location.href = option.path }, 500)
                }}>
