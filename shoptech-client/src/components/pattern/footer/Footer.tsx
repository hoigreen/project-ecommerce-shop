import { Container } from '@mui/material';
import React from 'react';

const Footer = () => {
  return (
    <>
      <div className="w-full min-h-200 bg-white mt-30 mb-10">
        <Container maxWidth="xl">
          <div className="flex flex-wrap w-full md:flex-nowrap lg:flex-nowrap xl:flex-nowrap">
            <div className="w-1/2 md:mr-2 md:w-1/4 lg:w-1/4 xl:w-1/4">
              <div className="block w-full text-sm pb-6 pl-4">
                <label className="text-xs font-bold block my-1 md:text-md md:mb-4 md:mt-8 md:my-2">
                  Tìm chi nhánh cửa hàng ShopTECH
                </label>
                <iframe
                  className="mt-4 w-full rounded-md border-gray-100"
                  src="https://www.google.com/maps/embed?p=!1m18!1m12!1m3!1d3918.2946148745355!2d106.6138108750036!3d10.865181857550368!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x31752a10b0f0554f%3A0x769800e8967d6703!2zNzAgVMO0IEvDvSwgVMOibiBDaMOhbmggSGnhu4dwLCBRdeG6rW4gMTIsIFRow6BuaCBwaOG7kSBI4buTIENow60gTWluaCwgVmlldG5hbQ!5e0!3m2!1sen!2s!4v1689436941428!5m2!1sen!2s"
                  allowFullScreen={true}
                  loading="lazy"
                  referrerPolicy="no-referrer-when-downgrade"
                ></iframe>
              </div>
              <div className="block w-full text-sm pb-6 pl-4">
                <label className="text-xs font-bold block my-1 md:text-md md:mb-4 md:mt-8 md:my-2">
                  Những phương thức thanh toán
                </label>
                <img
                  src="https://image.cellphones.com.vn/x35/media/logo/payment/moca-logo.png"
                  width="46"
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://image.cellphones.com.vn/x35/media/logo/payment/zalopay-logo.png"
                  width="46"
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://image.cellphones.com.vn/x35/media/logo/payment/vnpay-logo.png"
                  width="46"
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://image.cellphones.com.vn/x35/media/logo/payment/mpos-logo.png"
                  width="46"
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://fptshop.com.vn/Content/v4/images/ft-img6.png?v=1"
                  width="46"
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://fptshop.com.vn/Content/v4/images/ft-img7.png?v=1"
                  width="46"
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://fptshop.com.vn/Content/v4/images/ft-img9.png?v=1"
                  width="46"
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://fptshop.com.vn/Content/v4/images/ft-img14.png?v=1"
                  width="46"
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://fptshop.com.vn/Content/v4/images/ft-img8.png?v=1"
                  width="46"
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
              </div>
            </div>

            <div className="w-1/2 md:mr-2 md:w-1/4 lg:w-1/4 xl:w-1/4">
              <div className="block w-full text-sm pb-6 pl-4">
                <label className="text-xs font-bold block my-1 md:text-md md:mb-4 md:mt-8 md:my-2">
                  Thông tin, chính sách và điều khoản sử dụng
                </label>
                <a
                  href="/policy"
                  className="block decoration-none text-xs text-gray-700 my-2 py-2 md:text-sm"
                >
                  Mua hàng và thanh toán
                </a>
                <a
                  href="/policy"
                  className="block decoration-none text-xs text-gray-700 my-2 py-2 md:text-sm"
                >
                  Mua trả góp sản phẩm
                </a>
                <a
                  href="/policy"
                  className="block decoration-none text-xs text-gray-700 my-2 py-2 md:text-sm"
                >
                  Tra cứu thông tin đơn hàng
                </a>
                <a
                  href="/policy"
                  className="block decoration-none text-xs text-gray-700 my-2 py-2 md:text-sm"
                >
                  Tra cứu ưu đãi
                </a>
                <a
                  href="/policy"
                  className="block decoration-none text-xs text-gray-700 my-2 py-2 md:text-sm"
                >
                  Tra cứu thông tin bảo hành
                </a>
                <a
                  href="/policy"
                  className="block decoration-none text-xs text-gray-700 my-2 py-2 md:text-sm"
                >
                  Tra cứu hóa đơn điện tử
                </a>
                <a
                  href="/policy"
                  className="block decoration-none text-xs text-gray-700 my-2 py-2 md:text-sm"
                >
                  Chính sách bảo hành
                </a>
                <a
                  href="/policy"
                  className="block decoration-none text-xs text-gray-700 my-2 py-2 md:text-sm"
                >
                  Chính sách hậu mãi
                </a>
                <a
                  href="/policy"
                  className="block decoration-none text-xs text-gray-700 my-2 py-2 md:text-sm"
                >
                  Điều khoản sử dụng
                </a>
              </div>
            </div>

            <div className="w-1/2 md:mr-2 md:w-1/4 lg:w-1/4 xl:w-1/4">
              <div className="block w-full text-sm pb-6 pl-4">
                <label className="text-xs font-bold block my-1 md:text-md md:mb-4 md:mt-8 md:my-2">
                  Tổng đài viên hỗ trợ 24/7{' '}
                  <span className="font-semibold text-red-500">
                    (Miễn phí tư vấn)
                  </span>
                </label>
                <div className="block decoration-none text-xs text-gray-700 my-1 py-2 md:my-4 md:text-sm">
                  <p className="inline w-full mr-1 md:mr-4">Hỗ trợ mua hàng:</p>
                  <span className="inline font-semibold text-blue-600 text-xs pr-2 md:pr-4 md:text-sm">
                    1800 3234
                  </span>
                  <span className="hidden md:inline lg:inline xl:inline">
                    (24/7)
                  </span>
                </div>
                <div className="block decoration-none text-xs text-gray-700 my-1 py-2 md:my-4 md:text-sm">
                  <p className="inline w-full mr-1 md:mr-4">Hỗ trợ kỹ thuật:</p>
                  <span className="inline font-semibold text-blue-600 text-xs pr-2 md:pr-4 md:text-sm">
                    1800 1234
                  </span>
                  <span className="hidden md:inline lg:inline xl:inline">
                    (Giờ hành chính)
                  </span>
                </div>
                <div className="block decoration-none text-xs text-gray-700 my-1 py-2 md:my-4 md:text-sm">
                  <p className="inline w-full mr-1 md:mr-4">
                    Giải quyết khiếu nại:
                  </p>
                  <span className="inline font-semibold text-blue-600 text-xs pr-2 md:pr-4 md:text-sm">
                    1800 4321
                  </span>
                  <span className="hidden md:inline lg:inline xl:inline">
                    (24/7)
                  </span>
                </div>
                <div className="block decoration-none text-xs text-gray-700 my-1 py-2 md:my-4 md:text-sm">
                  <p className="inline w-full mr-1 md:mr-4">
                    Bảo hành sản phẩm:
                  </p>
                  <span className="inline font-semibold text-blue-600 text-xs pr-2 md:pr-4 md:text-sm">
                    1800 3234
                  </span>
                  <span className="hidden md:inline lg:inline xl:inline">
                    (Giờ hành chính)
                  </span>
                </div>
                <div className="block decoration-none text-xs text-gray-700 my-1 py-2 md:my-4 md:text-sm">
                  <p className="inline w-full mr-1 md:mr-4">Vấn đề khác:</p>
                  <span className="inline font-semibold text-blue-600 text-xs pr-2 md:pr-4 md:text-sm">
                    1800 3234
                  </span>
                  <span className="hidden md:inline lg:inline xl:inline">
                    (Giờ hành chính)
                  </span>
                </div>
              </div>
            </div>

            <div className="w-1/2 md:mr-2 md:w-1/4 lg:w-1/4 xl:w-1/4">
              <div className="block w-full text-sm pb-6 pl-4">
                <label className="text-xs font-bold block my-1 md:text-md md:mb-4 md:mt-8 md:my-2">
                  KẾT NỐI VỚI CHÚNG TÔI - ShopTECH BESIDE YOU
                </label>
                <img
                  src="https://image.cellphones.com.vn/44x/media/logo/social/cellphones-facebook.png"
                  width="46"
                  style={{ padding: '0 2px' }}
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://image.cellphones.com.vn/44x/media/logo/social/cellphones-zalo.png"
                  width="46"
                  style={{ padding: '0 2px' }}
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://image.cellphones.com.vn/44x/media/logo/social/cellphones-youtube.png"
                  width="46"
                  style={{ padding: '0 2px' }}
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://image.cellphones.com.vn/44x/media/logo/social/cellphones-tiktok.png"
                  width="46"
                  style={{ padding: '0 2px' }}
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://image.cellphones.com.vn/44x/media/logo/social/cellphones-instagram.png"
                  width="46"
                  style={{ padding: '0 2px' }}
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="data:image/png;base64,"
                  width="46"
                  style={{ padding: '0 2px' }}
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img />
              </div>

              <div className="block w-full text-sm pb-6 pl-4">
                <label
                  className="text-xs font-bold block my-1 md:text-md md:mb-4 md:mt-8 md:my-2"
                  style={{ width: '100%' }}
                >
                  Chứng nhận
                </label>
                <img
                  src="https://fptshop.com.vn/Content/v4/images/ft-img1.png?v=1"
                  width={40}
                  style={{ padding: '0 2px' }}
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://fptshop.com.vn/Content/v4/images/ft-img2.png?v=1"
                  width={100}
                  style={{ padding: '0 2px' }}
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://fptshop.com.vn/Content/v4/images/ft-img3.png?v=1"
                  width={80}
                  style={{ padding: '0 2px' }}
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://fptshop.com.vn/Content/v4/images/ft-img4.png?v=1"
                  width={80}
                  style={{ padding: '0 2px' }}
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
                <img
                  src="https://fptshop.com.vn/Content/v4/images/ft-img5.png?v=1"
                  width={80}
                  style={{ padding: '0 2px' }}
                  className="inline-block mr-2 mb-2 w-1/6 object-contain"
                />
              </div>
            </div>
          </div>
        </Container>
      </div>

      <div className="w-full bg-transparent mb-20">
        <p className="text-xs text-center w-full px-2 md:text-sm">
          ©️ Bản quyền thuộc ShopTECH - năm 2023 <br />
          Địa chỉ: 70 Tô Ký, phường Tân Chánh Hiệp. Quận 12, Thành phố Hồ Chí
          Minh.
        </p>
      </div>
    </>
  );
};

export default Footer;
